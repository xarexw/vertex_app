from django.contrib.auth.models import AbstractUser
from django.db import models
class CustomUser(AbstractUser):
    phone_number = models.CharField(max_length=20, blank=True, null=True, verbose_name="Телефон")
    is_manager = models.BooleanField(default=False, verbose_name="Це менеджер/тренер")
    
    bio = models.TextField(blank=True, verbose_name="Досвід / Про себе")

    def __str__(self):
        return f"{self.username} ({self.first_name} {self.last_name})"

#  cтатистика клієнта (вона у тебе вже була)
class ClientStat(models.Model):
    user = models.OneToOneField(CustomUser, on_delete=models.CASCADE, related_name='stats')
    weight = models.FloatField(verbose_name="Вага (кг)", default=0.0)
    height = models.FloatField(verbose_name="Зріст (см)", default=0.0)
    # сюди можна додати інші параметри
    updated_at = models.DateTimeField(auto_now=True)

    def __str__(self):
        return f"Stat for {self.user.username}"

# cертифікати тренера
class TrainerCertificate(models.Model):
    trainer = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name='certificates')
    title = models.CharField(max_length=200, verbose_name="Назва сертифікату")
    issued_by = models.CharField(max_length=100, verbose_name="Ким виданий")
    date_issued = models.DateField(verbose_name="Дата видачі")

    def __str__(self):
        return self.title